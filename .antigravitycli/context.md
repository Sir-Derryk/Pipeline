...
* **Вопрос 4.2 (Спецификация экспорта RAG JSON)**: Утвержден экспорт в виде единого иерархического JSON-файла (`.json`), повторяющего структуру кода (Module -> Class -> Methods/Properties). Каждая сущность в иерархии должна содержать расширенные метаданные: тип сущности (`entity_type`), полное имя (`fully_qualified_name`), диапазон строк (`line_range`), хэш сигнатуры (`signature_hash`) и связи/зависимости (`dependencies`).
