...
* **Вопрос 3 (Конфигурация и локальная автоматизация сборки)**: Согласована децентрализованная структура конфигурации в папке `ude/` в соответствии со структурой реальных проектов. Глобальный файл настроек движка `ude_global.json` задает системные пути (включая путь к `doxygen_path`), журналирование (`log_level`, `log_file`) и кэш, а также политику отказоустойчивости `error_policy` (`fail-fast` или `continue-on-error`) при сборке нескольких проектов. Каждый продукт содержит `product.json` с метаданными документации, а каждый целевой API-проект (Target) содержит `ude_config.json`, определяющий параметры парсинга и стадию сбора/препроцессинга через абстрактный `BaseCollector` (например, тип `cpp_doxygen` со ссылкой на локальный `Doxyfile` и директорию временных XML, либо `native_sources` для прямого парсинга). Перед запуском процессов коллектор проводит валидацию окружения (`validate_environment` — проверяет наличие Python, Doxygen, необходимых конфигураций, существование исходных каталогов, а также физическое наличие необходимых исходных файлов для сборки конкретного проекта). Для гибкого и гранулярного запуска локально и в CI/CD автоматизация вызовов выстроена на трёх уровнях: центральный командный файл в папке `engine/generate_all.bat` (вызывает сборку последовательно для всех проектов), файлы для каждого отдельного SDK/продукта (например, `generate_bimnv.bat`), и файлы для каждого конкретного проекта/таргета (например, `generate_docs.bat` или `generate_bimnv_api_cpp.bat`). Временные файлы Doxygen автоматически зачищаются коллектором сразу после парсинга через блок `try...finally` в оркестраторе. Поддерживаемые форматы вывода на MVP: `HugoXML` и `HTML`. Для сквозного тестирования решено использовать полноценные исходные коды (C++, C#, Java, Python), размещаемые в `engine/test_assets/sources/`.
