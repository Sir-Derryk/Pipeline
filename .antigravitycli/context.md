...
* **Вопрос 2.1 (Синхронизация кэша перевода в Git)**: Конфликты слияния и гонки коммитов в CI/CD устраняются путем ролевого разграничения прав доступа. Для рядовых разработчиков (в feature-ветках) парсинг и перевод осуществляются в режиме *Read-Only* (чтение кэша из репозитория без отправки коммитов). Обновление и запись в кэш переводов Git допускаются исключительно из-под авторизованной учетной записи Менеджера по переводам (или специализированного бота) с использованием защищенных CI/CD секретов/токенов. Дополнительно файлы кэша защищаются через механизм Git `CODEOWNERS`.
...
* **Вопрос 2.3 (Асинхронная верификация и поддержка XLIFF)**: Верификация локализации менеджером асинхронна и не блокирует разработку. ИИ генерирует черновики (`draft`) в фоне, причем генерация перевода запускается строго после успешного слияния (merge) кода в ветку **`master`**, что исключает затраты на перевод временного или чернового кода из feature-веток. В продакшене до верификации используется fallback (оригинальный английский) или плашка «AI-translated draft». Менеджер по переводам может экспортировать оригинальный английский текст и черновики в формат XLIFF (`.xlf`), передавать их переводчикам и импортировать обратно, автоматически утверждая переводы (`verified`).
* **Вопрос 3.1 (Интерфейс командной строки UDE)**: Утвержден гибридный CLI (команда-оркестратор `ude run`/`compile` совместно с низкоуровневыми модулями `parse`, `translate`, `render` и командами `translation export/import`). Все промежуточные JSON-файлы (IR, кэш перевода) сохраняются на диск в сжатом Gzip-формате (`.json.gz`). При запуске оркестратор автоматически распаковывает их в память, а перед выходом сжимает обратно, обеспечивая минимальный объем Git-репозитория и предотвращая замусоривание коммитов тяжелыми текстовыми JSON.
* **Вопрос 3.2 (Технологии ядра)**: Утверждены библиотека **Pydantic v2** для строгой, типизированной и сверхбыстрой валидации/сериализации IR и кэшей, а также шаблонизатор **Jinja2** как мощный промышленный стандарт для компиляции документации.
...
## 🗺️ Текущий шаг
Бизнес-требования (BRD) полностью согласованы и утверждены. Ожидание указаний пользователя для перехода к детальной проработке функциональных требований SRS (Software Requirements Specification) и архитектурного дизайна SDD (Software Design Document) на основе новых бизнес-требований.
