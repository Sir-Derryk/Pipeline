...
- [x] SDD (Software Design Document) — *Обновлен и детализирован (добавлены контракты классов BaseParser/BaseRenderer и спецификация CLI-интерфейса/ude.toml)*
...
* **Вопрос 4.1 (Аудит покрытия и Quality Gate)**: Контроль качества (Quality Gate) в CI/CD и независимый аудит покрытия (`ude coverage`) строго разделены. В знаменатель покрытия входят все публичные и защищенные API-сущности (классы, методы, поля и др.). Автоматически исключаются переопределенные системные методы (например, `ToString`, `__str__`) и тривиальные свойства (например, C# авто-свойства). Элемент считается документированным только при наличии текстового описания и детального описания всех его параметров и возвращаемого значения. Поддерживаются теги игнорирования (`DOM-IGNORE-BEGIN`/`DOM-IGNORE-END`, `\cond`/`\endcond`, `\internal`).
* **Вопрос 4.2 (Спецификация экспорта RAG JSON) [ОТЛОЖЕНО - БУДУЩИЕ ФАЗЫ (v2.0+)]**: Утвержден экспорт в виде единого иерархического JSON-файла (`.json`), повторяющего структуру кода (Module -> Class -> Methods/Properties). Каждая сущность в иерархии должна содержать расширенные метаданные: тип сущности (`entity_type`), полное имя (`fully_qualified_name`), диапазон строк (`line_range`), хэш сигнатуры (`signature_hash`) и связи/зависимости (`dependencies`). (Данный функционал отложен до будущих фаз).
