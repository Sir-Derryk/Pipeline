            ...
            for link in extractor.links:
                ...
                # 2. Проверка внутренних ссылок
                    ...
                    if clean_link.startswith("/"):
                        ...
                        link_path = clean_link.lstrip("/")
                        for prefix in ["ude-user-docs/api", "ude-user-docs"]:
                            if link_path.startswith(prefix):
                                link_path = link_path[len(prefix):].lstrip("/")
                        target_path = os.path.join(local_dir, link_path)
                    else:
